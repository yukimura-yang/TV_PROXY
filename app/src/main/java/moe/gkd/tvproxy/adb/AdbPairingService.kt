...
import moe.gkd.tvproxy.R
import moe.gkd.tvproxy.ShizukuSettings
...
class AdbPairingService : Service() {
    ...
    companion object {
        ...
        private fun replyIntent(context: Context, port: Int): Intent {
            return Intent(context, AdbPairingService::class.java).setAction(replyAction).putExtra(
                portKey, port)
        }
    }
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        val notification = when (intent?.action) {
            ...
            replyAction -> {
                val code = RemoteInput.getResultsFromIntent(intent)?.getCharSequence(
